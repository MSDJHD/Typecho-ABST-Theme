...
<div id="comments">
    ...
    <?php if ($comments->have()): ?>
        <h3 class="display-6"><?php $this->commentsNum(_t('暂无评论'), _t('仅有一条评论'), _t('已有 %d 条评论')); ?></h3>
    ...
    <?php if ($this->allow('comment')): ?>
        <div id="<?php $this->respondId(); ?>" class="respond">
            ...
            <h3 id="response" class="display-6"><?php _e('添加新评论'); ?></h3>
            <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" role="form">
                <?php if ($this->user->hasLogin()): ?>
                    <p><?php _e('登录身份: '); ?><a class="text-secondary"
    ...
    <?php else: ?>
        <h3 class="display-6"><?php _e('评论已关闭'); ?></h3>
    <?php endif; ?>
</div>
